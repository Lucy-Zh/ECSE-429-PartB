...
public class ProjectStepDefinitions {
  ...
  public void user_requests_to_retrieve_a_specific_project_with_id(String projectId) {
    ...
    response =
        RestAssured.get(
            BASE_URL
                + "/"
                + response.jsonPath().getList("projects.id").get(Integer.parseInt(projectId)));
    responseContext.setResponse(response);
  }
  ...
  public void user_requests_create_new_project_with_title(String title) {
    ...
    Response response =
        RestAssured.given().contentType("application/json").body(body).post(BASE_URL);
    responseContext.setResponse(response);
  }
  ...
  public void user_requests_create_new_project_with_request_body(String request_body) {
    Response response =
        RestAssured.given().contentType("application/json").body(request_body).post(BASE_URL);
    responseContext.setResponse(response);
  }
  ...
  public void user_requests_edit_project_with_new_completed_status(
      int project_id, String new_complete_status) {
    List<String> idList = RestAssured.get(BASE_URL).jsonPath().getList("projects.id");
  ...
  public void user_requests_delete_project_with_no_title(int projectId) {
    List<String> idList =
        RestAssured.get(BASE_URL)
            .jsonPath()
            .getList("projects.findAll { it.title == \"null\" }.id");
    Response response =
        RestAssured.given().contentType("application/json").delete(BASE_URL + "/" + idList.get(0));
    responseContext.setResponse(response);
  }
  ...
  public void user_requests_delete_nonexistent_project(int projectId) {
    Response response =
        RestAssured.given().contentType("application/json").delete(BASE_URL + "/" + projectId);
    responseContext.setResponse(response);
  }
  ...
  @When(
      "user requests to create a task within project {int} with title {string} and description"
          + " {string}")
  public void user_requests_create_task_with_title_and_description(
      int project_id, String title, String description) {
    List<String> idList = RestAssured.get(BASE_URL).jsonPath().getList("projects.id");
  ...
  public void a_list_of_all_projects_in_the_system_and_its_details_should_be_displayed() {
    List<Map<String, Object>> projects =
        responseContext.getResponse().jsonPath().getList("projects.id");
  ...
  public void a_specific_message_is_displayed(String message) {
    String actualError =
        (String) responseContext.getResponse().jsonPath().getList("errorMessages").get(0);
