...
public class TodosStepDefinitions {
  ...
  public void user_requests_create_new_todo_with_done_status(String done_Status) {
    Response response =
        RestAssured.given().contentType("application/json").body(done_Status).post(BASE_URL);
    responseContext.setResponse(response);
  }
  ...
  public void user_requests_edit_todo_with_new_done_status(int todo_id, String new_done_status) {
    List<String> idList = RestAssured.get(BASE_URL).jsonPath().getList("todos.id");
    String request_body =
        String.format("{\"doneStatus\": %s}", Boolean.parseBoolean(new_done_status));
    Response response =
        RestAssured.given()
            .contentType("application/json")
            .body(request_body)
  ...
  public void user_requests_edit_todo_with_id(int todo_id) {
    Response response =
        RestAssured.given().contentType("application/json").post(BASE_URL + "/" + todo_id);
    responseContext.setResponse(response);
  }
  ...
  public void user_requests_delete_todo_with_invalid_id(int todoId) {
    Response response =
        RestAssured.given().contentType("application/json").delete(BASE_URL + "/" + todoId);
    responseContext.setResponse(response);
  }
